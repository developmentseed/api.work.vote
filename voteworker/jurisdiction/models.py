...
class Jurisdiction(models.Model):
    ...
    registration_status = models.TextField('Registered in the state OR registered in the jurisdiction? - Input: S or J', null=True, blank=True)
    pre_registration = models.TextField('If pre-registration is available, is it required for 16/17 yo\'s? - Input: Y or N', null=True, blank=True)
    ...
    split_days_allowed = models.TextField('If split days are allowed, must one find a buddy? Input: Y or N', null=True, blank=True)
    compensation = models.TextField('Pay/compensation for the day - Input: Total amount OR hourly rate OR a range from min to max if the pay varies based on position.', null=True, blank=True)
    ...
    complete_training = models.TextField('Complete training for each election? - Input: Y or N', null=True, blank=True)
    post_training_exam = models.TextField('Pass a post-training exam or assessment - Input: Y or N', null=True, blank=True)
    must_have_email = models.TextField('Must have an email address and access to a computer and internet - Input: Y or N', null=True, blank=True)
    candidate_prohibition = models.TextField('Prohibition on being a candidate or related to a candidate - Y or N', null=True, blank=True)
